...
for i = 1:NTrain
    ...
    if i > 1
        ...
        contextIntensity = contextIntensity * g(ti-lastEventTime);
    end
    socialIntensity(outedges{ui},pi) = socialIntensity(outedges{ui},pi) + ...
        tau(ui,outedges{ui})'*g(0);
    contextIntensity(ui,:) = contextIntensity(ui,:)+phi(ui)*alpha(pi,:) * g(0);
    fprintf('%d of %d trainEvents analyzed.\n',i,NTrain);
end
%%
...
%              P,inedges,eventsMatrix,tau,g);
        ...
        for i=1:NumGeneratingPoints
            generatedPoints(i) = generatePoint(t0,baseIntensity,socialIntensityInitial,contexIntensityInitial,g);
        end
...
for i = 1:NTest
    ...
    contextIntensity(ui,:) = contextIntensity(ui,:)+phi(ui)*alpha(pi,:) * g(0);
    fprintf('%d of %d testEvents analyzed.\n',i,NTest);
end
...
function [time] = generatePoint(t0,baseIntensity,...
    socialIntensityInitial, contextIntensityInitial, g)
time = t0;
isGeneratedPoint = false;
while (isGeneratedPoint == false)
    I = baseIntensity+socialIntensityInitial*g(time-t0)+contextIntensityInitial*g(time-t0);
    time = time+exprnd(1/I);
    Is = baseIntensity+socialIntensityInitial*g(time-t0)+contextIntensityInitial*g(time-t0);
